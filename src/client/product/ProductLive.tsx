...
function LoadingScreen() {
  return <div className="product-screen product-screen--matchmaking"><div className="product-grid" aria-hidden="true" /><Brand /><section className="queue-card" role="status" aria-live="polite" aria-busy="true"><div className="queue-orbit" aria-hidden="true"><span /></div><p className="product-eyebrow">ACCOUNT SESSION</p><h1>Loading pilot</h1></section></div>;
}
...
function AuthLive({ account }: Readonly<{ account: ReturnType<typeof useAccount> }>) {
  ...
  return (
    <div className="product-screen product-screen--auth">
      ...
      <section className="auth-card" aria-label="Account access" aria-busy={busy}>
        ...
        <div className="segmented-control" role="group" aria-label="Authentication mode">
          <button type="button" aria-pressed={mode === "login"} className={mode === "login" ? "is-active" : ""} onClick={() => setMode("login")}>LOGIN</button>
          <button type="button" aria-pressed={mode === "register"} className={mode === "register" ? "is-active" : ""} onClick={() => setMode("register")}>REGISTER</button>
        </div>
        <label className="product-field"><span>USER ID</span><input name="username" value={loginId} onChange={(event) => setLoginId(event.target.value)} autoComplete="username" placeholder="pilot-id" /></label>
        {mode === "register" && <label className="product-field"><span>DISPLAY NAME</span><input name="displayName" value={displayName} onChange={(event) => setDisplayName(event.target.value)} autoComplete="nickname" placeholder="display name" /></label>}
        <label className="product-field"><span>PASSWORD</span><input name="password" value={password} onChange={(event) => setPassword(event.target.value)} type="password" autoComplete={mode === "login" ? "current-password" : "new-password"} placeholder="10+ characters" onKeyDown={(event) => { if (event.key === "Enter" && !busy) void submit(); }} /></label>
        <button type="button" className="product-primary" disabled={busy} onClick={() => void submit()}>{busy ? "PLEASE WAIT" : mode === "login" ? "LOGIN" : "CREATE ACCOUNT"}</button>
...
function HomeLive({ profile, account, onStart, onLeaderboard }: Readonly<{
  ...
  return (
    <div className="product-screen product-screen--home">
      ...
      <header className="home-topbar"><Brand /><div className="pilot-chip" aria-label={`Pilot ${profile.displayName}, rating ${profile.rating}`}><span>{profile.displayName}</span><strong>{profile.rating}</strong><button type="button" onClick={() => void account.logout()}>SIGN OUT</button></div></header>
      <main className="home-layout">
        <section className="home-hero">
          ...
          <button type="button" className="start-button" onClick={onStart}><span>START</span><small>BEGIN MATCHMAKING</small></button>
          {account.errorMessage && <p className="preview-note" role="alert">{account.errorMessage}</p>}
        </section>
        <section className="home-cards" aria-label="Pilot summary">
          ...
          <article className="summary-card">
            ...
            {fixed && <button type="button" className="product-secondary" onClick={() => void account.setFixedAircraft(null)}>RETURN TO RANDOM</button>}
            {!fixed && fixable && <button type="button" className="product-secondary" onClick={() => void account.setFixedAircraft(fixable.aircraftId)}>FIX {fixable.displayName.toUpperCase()}</button>}
          </article>
          <button type="button" className="summary-card summary-card--button" onClick={onLeaderboard}><span>LEADERBOARD</span><strong>VIEW</strong><small>RANKINGS & RECORDS</small></button>
        </section>
      </main>
...
function MatchmakingLive({ fixedAircraftId, onMatched, onCancel }: Readonly<{
  ...
  return (
    <div className="product-screen product-screen--matchmaking"><div className="product-grid" aria-hidden="true" /><Brand />
      <section className="queue-card"><div className="queue-orbit" aria-hidden="true"><span /></div><p className="product-eyebrow">MATCHMAKING // LIVE</p><h1 role="status" aria-live="polite">{matchmaking.status === "connecting" ? "Connecting to queue" : "Searching for opponent"}</h1>
        <div className="queue-meta"><span>RATED 1V1</span><span>{fixedAircraftId ? "FIXED AIRCRAFT" : "RANDOM AIRCRAFT"}</span><span aria-label={`Elapsed ${elapsedSeconds} seconds`}>{String(Math.floor(elapsedSeconds / 60)).padStart(2, "0")}:{String(elapsedSeconds % 60).padStart(2, "0")}</span></div>
        {matchmaking.status === "error" && <p className="preview-note" role="alert">{matchmaking.errorMessage}</p>}<button type="button" className="product-secondary" onClick={() => { matchmaking.cancel(); onCancel(); }}>CANCEL</button>
      </section>
    </div>
  );
}
...
function CountdownLive({ assignment, onActive }: Readonly<{ assignment: MatchFoundAssignment; onActive: () => void }>) {
  ...
  return <div className="product-screen product-screen--matchmaking"><div className="product-grid" aria-hidden="true" /><Brand /><section className="queue-card"><p className="product-eyebrow">MATCH {assignment.matchId.slice(0, 8).toUpperCase()}</p><h1 role="status" aria-live="polite" aria-atomic="true">{Math.max(1, Math.ceil(remainingMs / 1_000))}</h1><p className="product-copy">Room {assignment.roomCode} // synchronized start</p><div className="queue-meta"><span>100 HP</span><span>04:00</span><span>{assignment.spawnSide.toUpperCase()} SIDE</span></div></section></div>;
}
...
function MatchLive({ assignment, onResolved }: Readonly<{ assignment: MatchFoundAssignment; onResolved: (outcome: ProductMatchOutcome) => void }>) {
  ...
  const actionFeedback = actionFeedbackLabel(ranked.lastActionFeedback?.code);
  return <div className="product-match-shell"><FlightRuntime showDevelopmentPanels={false} externalNetworkController={ranked} stagingSlot={stagingSlot} /><div className="c4-match-hud"><div className="match-hud__top"><div className="hp-block hp-block--local" aria-label={`Your Heart Points ${localHp}`}><span>YOU // {aircraft?.displayName ?? assignment.aircraftId}</span><strong>{localHp}</strong><div role="progressbar" aria-label="Your Heart Points" aria-valuemin={0} aria-valuemax={MATCH_RULES.startingHeartPoints} aria-valuenow={localHp}><i style={{ width: `${localHp}%` }} /></div></div><div className="match-clock" aria-label={`${phase === "overtime" ? "Overtime" : phase === "active" ? "Regulation" : "Syncing"}, ${formatClock(remaining)} remaining`}><span>{phase === "overtime" ? "OVERTIME" : phase === "active" ? "REGULATION" : "SYNCING"}</span><strong>{formatClock(remaining)}</strong><small>{assignment.roomCode}</small></div><div className="hp-block hp-block--peer" aria-label={`Peer Heart Points ${peerHp}`}><span>PEER</span><strong>{peerHp}</strong><div role="progressbar" aria-label="Peer Heart Points" aria-valuemin={0} aria-valuemax={MATCH_RULES.startingHeartPoints} aria-valuenow={peerHp}><i style={{ width: `${peerHp}%` }} /></div></div></div><button type="button" className="match-action-state" style={{ pointerEvents: "auto", color: "inherit", font: "inherit", cursor: actionReady ? "pointer" : "default" }} onClick={ranked.activateAction} disabled={!actionReady} aria-label={`${actionModule?.displayName ?? "Abstract action module"}: ${actionStatus}. ${actionFeedback}`}><span>ACTION // {actionModule?.displayName ?? "ABSTRACT MODULE"}</span><strong>{actionStatus}</strong><small role="status" aria-live="polite">{actionFeedback}</small></button><button type="button" className="match-exit-preview" onClick={ranked.leaveMatch} disabled={Boolean(state?.result)}>FORFEIT MATCH</button></div></div>;
}

function ResultLive({ outcome, onHome }: Readonly<{ outcome: ProductMatchOutcome; onHome: () => void }>) {
  return <div className="product-screen product-screen--matchmaking"><div className="product-grid" aria-hidden="true" /><Brand /><section className="queue-card" role="status" aria-live="polite"><p className="product-eyebrow">MATCH COMPLETE</p><h1>{outcome.result.toUpperCase()}</h1><p className="product-copy">{outcome.reason.replaceAll("-", " ").toUpperCase()} // HP {outcome.localHeartPoints}–{outcome.peerHeartPoints}</p><button type="button" className="product-primary" onClick={onHome}>RETURN HOME</button></section></div>;
}

function LeaderboardLive({ entries, onBack }: Readonly<{ entries: readonly LeaderboardProfile[]; onBack: () => void }>) {
  return <div className="product-screen product-screen--leaderboard"><div className="product-grid" aria-hidden="true" /><header className="home-topbar"><Brand /><button type="button" className="product-secondary" onClick={onBack}>BACK HOME</button></header><main className="leaderboard-card" aria-labelledby="leaderboard-title"><p className="product-eyebrow">GLOBAL RANKING</p><h1 id="leaderboard-title">Leaderboard</h1><div role="table" aria-label="Global pilot rankings"><div className="leaderboard-head" role="row"><span role="columnheader">RANK</span><span role="columnheader">PILOT</span><span role="columnheader">RATING</span><span role="columnheader">W/L/D</span></div>{entries.map((entry) => <div className="leaderboard-row" role="row" key={entry.userId}><strong role="cell">#{entry.rank}</strong><span role="cell">{entry.displayName}</span><strong role="cell">{entry.rating}</strong><span role="cell">{entry.wins}/{entry.losses}/{entry.draws}</span></div>)}</div>{entries.length === 0 && <p className="preview-note" role="status">No ranked pilots yet.</p>}</main></div>;
}
