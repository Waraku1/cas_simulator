...
const presence1 = await client1.waitFor(
  (m) => m?.type === "presence" && m.peerConnected === true && m.playerCount === 2,
  "two-player presence",
);
const presence2 = await client2.waitFor(
  (m) => m?.type === "presence" && m.peerConnected === true && m.playerCount === 2,
  "two-player presence",
);

if (!presence1.peerConnected || !presence2.peerConnected) {
  throw new Error("Peer presence did not converge");
}
