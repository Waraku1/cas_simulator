  ...
  return (
    <div className="product-screen product-screen--matchmaking">
      ...
      <section className="queue-card">
        ...
        <p className="product-eyebrow">ACCOUNT SESSION</p>
        <h1>Loading pilot</h1>
      </section>
    </div>
  );
...
function AuthLive({
  ...
  onRegister: (loginId: string, displayName: string, password: string) => Promise<boolean>;
}>) {
  ...
  const submit = async () => {
    if (mode === "login") await onLogin(loginId, password);
    else await onRegister(loginId, displayName, password);
  };
    ...
    <div className="product-screen product-screen--auth">
      ...
      <section className="auth-card" aria-label="Account access">
        ...
        <p className="product-copy">
          {mode === "login"
            ? "Sign in to enter matchmaking and load your rating, record, and fixed-aircraft state."
            : "Create one pilot account. Passwords are stored only as derived hashes; the browser receives an HttpOnly session cookie."}
        </p>
        ...
        <label className="product-field">
          <span>USER ID</span>
          <input value={loginId} onChange={(event) => setLoginId(event.target.value)} autoComplete="username" placeholder="pilot-id" />
        </label>
        {mode === "register" && (
          <label className="product-field">
            <span>DISPLAY NAME</span>
            <input value={displayName} onChange={(event) => setDisplayName(event.target.value)} autoComplete="nickname" placeholder="display name" />
          </label>
        )}
        <label className="product-field">
          <span>PASSWORD</span>
        ...
        <button className="product-primary" disabled={busy} onClick={() => void submit()}>
          {busy ? "PLEASE WAIT" : mode === "login" ? "LOGIN" : "CREATE ACCOUNT"}
        </button>
        {errorMessage && <p className="preview-note" role="alert">{errorMessage}</p>}
      </section>
    </div>
  );
}
...
function HomeLive({
  ...
  onLeaderboard: () => void;
  onSignOut: () => void;
  onSetFixed: (aircraftId: string | null) => Promise<boolean>;
}>) {
    ...
    <div className="product-screen product-screen--home">
      ...
      <main className="home-layout">
        <section className="home-hero">
          ...
          <p className="product-copy">
            Match with one opponent, receive a fictional sidegrade aircraft, and compete over a continuous 100 Heart Point match.
          </p>
        ...
        <section className="home-cards" aria-label="Pilot summary">
          <article className="summary-card summary-card--rating">
            <span>RATING</span>
            <strong>{profile.rating}</strong>
            <small>{profile.wins}W / {profile.losses}L / {profile.draws}D</small>
          </article>
          <article className="summary-card">
            <span>FIXED AIRCRAFT</span>
            <strong>{fixed?.displayName ?? "RANDOM"}</strong>
            <small>{fixed ? "PERSISTENT" : "ASSIGNED EACH MATCH"}</small>
            {fixed && <button className="product-secondary" onClick={() => void onSetFixed(null)}>RETURN TO RANDOM</button>}
            {!fixed && fixable && (
              <button className="product-secondary" onClick={() => void onSetFixed(fixable.aircraftId)}>
                FIX {fixable.displayName.toUpperCase()}
              </button>
            )}
          </article>
...
function MatchmakingLive({
  ...
  return (
    <div className="product-screen product-screen--matchmaking">
      ...
      <section className="queue-card">
        <div className="queue-orbit" aria-hidden="true"><span /></div>
        <p className="product-eyebrow">MATCHMAKING // LIVE</p>
        ...
        <button className="product-secondary" onClick={() => { matchmaking.cancel(); onCancel(); }}>CANCEL</button>
      </section>
    </div>
  );
}
...
function MatchLive({ assignment, onResolved }: Readonly<{
  ...
  useEffect(() => {
    const handle = (event: KeyboardEvent) => {
      ...
      if (event.target instanceof HTMLInputElement || event.target instanceof HTMLButtonElement || event.target instanceof HTMLTextAreaElement) return;
  ...
  const local = ranked.slot && state ? state.participants[ranked.slot - 1] : null;
  const peer = ranked.slot && state ? state.participants[ranked.slot === 1 ? 1 : 0] : null;
  const localHp = local?.heartPoints ?? MATCH_RULES.startingHeartPoints;
  const peerHp = peer?.heartPoints ?? MATCH_RULES.startingHeartPoints;
  const serverNow = clockNowMs + ranked.serverTimeOffsetMs;
  ...
  useEffect(() => {
    ...
    onResolved({
      ...
      localHeartPoints: localHp,
      peerHeartPoints: peerHp,
    });
  }, [state?.result, ranked.slot, localHp, peerHp, onResolved]);

  const phase = state?.phase ?? "countdown";
  const remaining = state
    ? phase === "overtime" ? state.overtimeEndsAtMs - serverNow
      : phase === "active" ? state.regulationEndsAtMs - serverNow
        : Math.max(0, state.activeAtMs - serverNow)
    : 0;
  const cooldownMs = local ? Math.max(0, local.nextActionAtMs - serverNow) : 0;
  const actionLabel = cooldownMs > 0 ? `COOLDOWN ${(cooldownMs / 1_000).toFixed(1)}s`
    : ranked.actionFeedback?.code === "accepted" ? "ACTION ACCEPTED"
      : ranked.actionFeedback?.code ? ranked.actionFeedback.code.replaceAll("_", " ").toUpperCase()
        : "READY // SPACE";

  return (
    <div className="product-match-shell">
      <FlightRuntime showDevelopmentPanels={false} networkController={ranked} stagingSide={assignment.spawnSide} />
      <div className="c4-match-hud" aria-label="Ranked match HUD">
        <div className="match-hud__top">
          <div className="hp-block hp-block--local">
            <span>YOU // {aircraft?.displayName ?? assignment.aircraftId}</span><strong>{localHp}</strong><div><i style={{ width: `${localHp}%` }} /></div>
          </div>
          <div className="match-clock"><span>{phase === "overtime" ? "OVERTIME" : phase === "active" ? "REGULATION" : "SYNCING"}</span><strong>{formatClock(remaining)}</strong><small>{ranked.status.toUpperCase()}</small></div>
          <div className="hp-block hp-block--peer">
            <span>PEER</span><strong>{peerHp}</strong><div><i style={{ width: `${peerHp}%` }} /></div>
          </div>
        </div>
        <div className="match-action-state"><span>{actionModule?.displayName ?? "ACTION"}</span><strong>{actionLabel}</strong><small>ABSTRACT MODULE</small></div>
        <button className="match-exit-preview" onClick={ranked.forfeit}>FORFEIT MATCH</button>
      </div>
    </div>
  );
}
...
function ResultLive({ outcome, onHome }: Readonly<{ outcome: ProductMatchOutcome; onHome: () => void }>) {
  return (
    <div className="product-screen product-screen--matchmaking">
      ...
      <Brand />
      <section className="queue-card">
        <p className="product-eyebrow">MATCH COMPLETE</p>
        <h1>{outcome.result.toUpperCase()}</h1>
        <p className="product-copy">{outcome.reason.replaceAll("-", " ").toUpperCase()} // HP {outcome.localHeartPoints}–{outcome.peerHeartPoints}</p>
        <button className="product-primary" onClick={onHome}>RETURN HOME</button>
      </section>
    </div>
  );
}
  ...
  return (
    <div className="product-screen product-screen--leaderboard">
      ...
      <main className="leaderboard-card">
        ...
        {entries.length === 0 && <p className="preview-note">No ranked pilots yet.</p>}
      </main>
    </div>
  );
}
...
export function ProductLive() {
  ...
  const [screen, setScreen] = useState<ProductScreen>("home");
  ...
  const [leaderboard, setLeaderboard] = useState<readonly LeaderboardProfile[]>([]);

  if (account.status === "loading") return <LoadingScreen />;
  if (!account.user) {
    return (
      <AuthLive
        busy={account.status === "submitting"}
  ...
  if (screen === "home") {
    return (
      <HomeLive
        profile={profile}
        errorMessage={account.errorMessage}
        onStart={() => { setAssignment(null); setOutcome(null); setScreen("matchmaking"); }}
        onLeaderboard={() => { void loadLeaderboard().then((entries) => { setLeaderboard(entries); setScreen("leaderboard"); }); }}
        onSignOut={() => void account.logout()}
        onSetFixed={account.setFixedAircraft}
      />
    );
  }
  if (screen === "matchmaking") {
    return <MatchmakingLive fixedAircraftId={profile.fixedAircraftId} onMatched={(next) => { setAssignment(next); setScreen("aircraft-assignment"); }} onCancel={() => setScreen("home")} />;
  }
  ...
  if (screen === "result" && outcome) {
    return <ResultLive outcome={outcome} onHome={() => { void account.refresh(); setScreen("home"); }} />;
  }
  if (screen === "leaderboard") return <LeaderboardLive entries={leaderboard} onBack={() => setScreen("home")} />;

  return <HomeLive profile={profile} errorMessage={account.errorMessage} onStart={() => setScreen("matchmaking")} onLeaderboard={() => setScreen("leaderboard")} onSignOut={() => void account.logout()} onSetFixed={account.setFixedAircraft} />;
}
